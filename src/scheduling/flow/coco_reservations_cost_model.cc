...
#include "scheduling/flow/coco_reservations_cost_model.h"
...
CocoReservationsCostModel::CocoReservationsCostModel(
  ...
  infinity_ = omega_ * (ReservationsCostVector_t::dimensions_ - 1) +
              MAX_PRIORITY_VALUE * omega_;
  // Shut up unused warnings for now
  CHECK_NOTNULL(leaf_res_ids_);
}

void CocoReservationsCostModel::AccumulateResourceStats(ResourceDescriptor* accumulator,
...
const string CocoReservationsCostModel::DebugInfo() const {
  string out;
...
const string CocoReservationsCostModel::DebugInfoCSV() const {
  string out;
  ResourceStatus* root_rs =
    FindPtrOrNull(*resource_map_,
                  ResourceIDFromString(
...
Cost_t CocoReservationsCostModel::FlattenCostVector(ReservationsCostVector_t cv) {
...
Cost_t CocoReservationsCostModel::FlattenInterferenceScore(
...
const TaskDescriptor& CocoReservationsCostModel::GetTask(TaskID_t task_id) {
  TaskDescriptor* td = FindPtrOrNull(*task_map_, task_id);
  CHECK_NOTNULL(td);
  return *td;
}

void CocoReservationsCostModel::GetInterferenceScoreForTask(
    TaskID_t task_id,
...
vector<EquivClass_t>* CocoReservationsCostModel::GetTaskEquivClasses(TaskID_t task_id) {
...
vector<EquivClass_t>* CocoReservationsCostModel::GetResourceEquivClasses(
    ResourceID_t res_id) {
  // Not implemented.
  return NULL;
}

vector<ResourceID_t>* CocoReservationsCostModel::GetOutgoingEquivClassPrefArcs(
    EquivClass_t ec) {
...
vector<TaskID_t>* CocoReservationsCostModel::GetIncomingEquivClassPrefArcs(
    EquivClass_t tec) {
...
vector<ResourceID_t>* CocoReservationsCostModel::GetTaskPreferenceArcs(TaskID_t task_id) {
  // TODO(malte): implement!
  return NULL;
}

pair<vector<EquivClass_t>*, vector<EquivClass_t>*>
    CocoReservationsCostModel::GetEquivClassToEquivClassesArcs(EquivClass_t tec) {
  // TODO(malte): implement!
...
// scheduling it.
Cost_t CocoReservationsCostModel::TaskToUnscheduledAggCost(TaskID_t task_id) {
  const TaskDescriptor& td = GetTask(task_id);
  // Baseline value (based on resource request)
  ReservationsCostVector_t cost_vector;
...
// aggregator.
Cost_t CocoReservationsCostModel::UnscheduledAggToSinkCost(JobID_t job_id) {
  return 0LL;
}
...
Cost_t CocoReservationsCostModel::TaskToClusterAggCost(TaskID_t task_id) {
  // Tasks may not use the cluster aggregator in the CoCo model
  return infinity_;
}

Cost_t CocoReservationsCostModel::TaskToResourceNodeCost(TaskID_t task_id,
...
Cost_t CocoReservationsCostModel::ResourceNodeToResourceNodeCost(
    ResourceID_t source,
  ...
  ReservationsCostVector_t cost_vector;
  bzero(&cost_vector, sizeof(ReservationsCostVector_t));
...
Cost_t CocoReservationsCostModel::LeafResourceNodeToSinkCost(ResourceID_t resource_id) {
  return 0LL;
}

Cost_t CocoReservationsCostModel::TaskContinuationCost(TaskID_t task_id) {
  // XXX(malte): not currently used.
  LOG(FATAL) << "Unimplememented!";
  return 0LL;
}

Cost_t CocoReservationsCostModel::TaskPreemptionCost(TaskID_t task_id) {
  // XXX(malte): not currently used.
  LOG(FATAL) << "Unimplememented!";
  return 0LL;
}

Cost_t CocoReservationsCostModel::TaskToEquivClassAggregator(TaskID_t task_id,
                                                 EquivClass_t tec) {
...
pair<Cost_t, int64_t> CocoReservationsCostModel::EquivClassToResourceNode(
    EquivClass_t ec,
    ResourceID_t res_id) {
...
Cost_t CocoReservationsCostModel::EquivClassToEquivClass(EquivClass_t tec1,
                                             EquivClass_t tec2) {
  return 0LL;
}

ResourceID_t CocoReservationsCostModel::MachineResIDForResource(ResourceID_t res_id) {
  ResourceStatus* rs = FindPtrOrNull(*resource_map_, res_id);
  CHECK_NOTNULL(rs);
...
uint32_t CocoReservationsCostModel::NormalizeCost(double raw_cost, double max_cost) {
  if (omega_ == 0 || max_cost == 0.0)
    return 0;
  return (raw_cost / max_cost) * omega_;
}

uint32_t CocoReservationsCostModel::NormalizeCost(uint64_t raw_cost, uint64_t max_cost) {
...
void CocoReservationsCostModel::PrintCostVector(ReservationsCostVector_t cv) {
...
string CocoReservationsCostModel::ResourceVectorToString(
...
void CocoReservationsCostModel::AddMachine(ResourceTopologyNodeDescriptor* rtnd_ptr) {
...
void CocoReservationsCostModel::AddTask(TaskID_t task_id) {
...
void CocoReservationsCostModel::RemoveMachine(ResourceID_t res_id) {
}

void CocoReservationsCostModel::RemoveTask(TaskID_t task_id) {
  vector<EquivClass_t>* equiv_classes = GetTaskEquivClasses(task_id);
...
FlowGraphNode* CocoReservationsCostModel::GatherStats(FlowGraphNode* accumulator,
                                          FlowGraphNode* other) {
  ...
  if (accumulator->type_ == FlowNodeType::PU) {
    ...
    if (have_sample) {
      ...
      rd_ptr->mutable_min_available_resources_below()->set_disk_bw(
          rd_ptr->resource_capacity().disk_bw() -
          (latest_stats.disk_bw() / BYTES_TO_MB));
      rd_ptr->mutable_available_resources()->set_disk_cap(
          rd_ptr->resource_capacity().disk_cap() -
          (latest_stats.disk_cap() / BYTES_TO_MB));
      rd_ptr->mutable_max_available_resources_below()->set_disk_cap(
          rd_ptr->resource_capacity().disk_cap() -
          (latest_stats.disk_cap() / BYTES_TO_MB));
      rd_ptr->mutable_min_available_resources_below()->set_disk_cap(
          rd_ptr->resource_capacity().disk_cap() -
          (latest_stats.disk_cap() / BYTES_TO_MB));
...
CocoReservationsCostModel::ResourceVectorFitIndication_t
CocoReservationsCostModel::CompareResourceVectors(
...
void CocoReservationsCostModel::PrepareStats(FlowGraphNode* accumulator) {
  ResourceStatus* rs_ptr =
    FindPtrOrNull(*resource_map_, accumulator->resource_id_);
  if (!rs_ptr)
    return;
...
uint64_t CocoReservationsCostModel::TaskFitCount(const ResourceVector& req,
...
CocoReservationsCostModel::TaskFitIndication_t
CocoReservationsCostModel::TaskFitsUnderResourceAggregate(
    EquivClass_t tec,
...
FlowGraphNode* CocoReservationsCostModel::UpdateStats(FlowGraphNode* accumulator,
                                          FlowGraphNode* other) {
  if (accumulator->type_ == FlowNodeType::ROOT_TASK ||
      accumulator->type_ == FlowNodeType::SCHEDULED_TASK ||
      accumulator->type_ == FlowNodeType::UNSCHEDULED_TASK ||
