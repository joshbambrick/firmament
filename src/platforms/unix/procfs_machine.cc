...
const MachinePerfStatisticsSample* ProcFSMachine::CreateStatistics(
  ...
  stats->set_disk_cap(disk_space_stats.available_space  / BYTES_TO_MB);

  // update disk_cap (usage)
...
void ProcFSMachine::GetMachineCapacity(ResourceVector* cap) {
  ...
  if (FLAGS_monitor_blockdev_capacity == -1) {
    ...
    DiskSpaceStatistics_t disk_space_stats = GetDiskSpaceStats();
    stats->set_disk_cap(disk_space_stats.capacity_space  / BYTES_TO_MB);
  } else {
    stats->set_disk_cap(FLAGS_monitor_blockdev_capacity);
  }

  // set this from a command line flag
}
