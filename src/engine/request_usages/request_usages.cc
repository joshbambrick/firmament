...
void RequestUsages::LookUpTree(Request request,
  ...
  if (tree_) {
    ...
    for (uint32_t i = 0; i < k_; ++i) {
      ANNpoint cur_point = request_data_points_[neighbour_indices_[i]];
      cout << "INDEX OF POINT IS: " << neighbour_indices_[i] << endl;
      cout << "0: " << cur_point[0] << endl;
      cout << "1: " << cur_point[1] << endl;
      cout << "2: " << cur_point[2] << endl;
...
void RequestUsages::AddToTree(UsageRecordList new_record_list) {
  ...
  if (rebuild_tree) {
    cout << "rebuild with new size " << record_list_size_ << endl;
